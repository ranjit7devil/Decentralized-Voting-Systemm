     ...
     instance.getCountCandidates().then(function(countCandidates){
          ...
          for (var i = 0; i < countCandidates; i++ ){
            instance.getCandidate(i+1).then(function(data){
              ...
              var viewCandidates = `<tr><td> <input class="form-check-input" type="radio" name="candidate" value="${id}" id=${id}>` + name + "</td><td>" + party + "</td><td class='rem_u'>" + voteCount + "</td></tr>"
...
window.addEventListener("load", function() {
  ...
  if (path === 'index.html'){
    const element = document.querySelector('.rem_u');
